...
class Matrix:
    ...
    def front_substitution(self, b):
        """
		TODO: Finish this.
		"""
        if b.rows_count() != 1:
            raise ValueError("B parameter must be vector.")
        if self.columns_count() != self.rows_count():
            raise ValueError("Matrix must be sqared for front substitution.")
    ...
    def backward_substitution(self, b):
        ...
        if b.columns_count() != self.rows_count():
            raise ValueError("B vector don't have same dimension as matrix.")

        for i in range(self.rows_count() - 1, -1, -1):
            if self[i, i] == 0 or abs(self[i, i]) <= 0.000001:
                raise ValueError("You can't divide with 0.")
            b[0, i] /= self[i, i]
            for j in range(i):
                b[0, j] -= self[j, i] * b[0, i]
    ...
    def solve(a, b, pivot=False):
        ...
        if pivot:
            ...
            a.front_substitution(bb)
            a.backward_substitution(bb)
        else:
            aa.lu_decomposition()
            aa.front_substitution(bb)
            aa.backward_substitution(bb)
    ...
    def inverse(a):
        ...
        tmp = get_i(a.columns_count()).to_list()
        ...
        for i in range(a.columns_count()):
            aaa = aa.__copy__()
            bb = Matrix([b[i]])
            aaa.front_substitution(bb)
            aaa.backward_substitution(bb)
            result.append(bb.to_list()[0])
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, numbers.Number):
            ...
            if self.columns_count() != other.rows_count():
                raise ValueError("Matrices don't have compatible dimensions.")
            for r in self.value:
                tmp = []
